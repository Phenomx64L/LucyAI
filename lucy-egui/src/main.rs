...
// ── LAS VISTAS, QUE SON `impl App` REPARTIDO ────────────────────────────────
//
// Cada `vista_*` continua el mismo `impl App` de este fichero: son cortes y pegas
// sin logica nueva. Lo que lo permite es que `App` se declara AQUI, en la raiz, y
// en Rust un campo privado de un modulo se ve desde todos sus descendientes.
mod vista_compliance;
mod vista_memoria;
mod vista_nexshell;
mod bombas;
mod vista_config;
mod vista_inventario;
mod vista_logviewer;
mod vista_paleta;
mod vista_workspace;
...
mod presupuesto {
    ...
    fn el_plazo_es_para_el_modelo_y_no_para_los_comandos() {
        ...
        //
        // LEE `bombas.rs`, NO ESTE FICHERO, desde que los recogedores se mudaron
        // ahi. Y el fallo fue instructivo: apuntando a `main.rs` el test no se
        // caia por no encontrar nada — se encontraba A SI MISMO, porque la aguja
        // «fn pump_nx_plazo» esta escrita tres lineas mas abajo. Partia por su
        // propio literal y examinaba el cuerpo del test en vez del de la guarda.
