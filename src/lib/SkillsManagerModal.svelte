<script>
  ...
  import { trad } from '$lib/i18n';
    ...
    async function generateSkillWithAI() {
        ...
        if (!idea) { aiError = $trad('Describe qué skill quieres generar'); return; }

        // Resolve model up-front and surface key/provider issues BEFORE spinning.
        const resolved = resolveAIModel();
        if (!resolved.hasKey) {
            aiError = $trad('Sin API keys configuradas. Abre Settings → API Key para agregar una (Gemini es gratis).');
            return;
        }
        ...
        try {
            ...
            const timeoutPromise = new Promise((_, reject) => setTimeout(
                () => reject(new Error($trad('Timeout del frontend (30s). Prueba un modelo más rápido o revisa tu conexión.'))),
                30000));
            ...
            if (!m) {
                throw new Error(($trad('La IA no devolvió JSON válido. Respuesta: '))
                    + txt.slice(0, 200));
            }
            ...
            try {
                ...
                throw new Error(($trad('Error al parsear JSON: '))
                    + parseErr.message + '\n\n' + ($trad('Respuesta IA: ')) + m[0].slice(0, 300));
            }
            ...
            toast($trad('Skill generada por IA — revisa y guarda'), 'success');
        } catch (e) {
            if (timerInterval) { clearInterval(timerInterval); timerInterval = null; }
            const msg = String(e?.message || e);
            error = msg;
            aiError = msg.slice(0, 600); // show fuller error in modal
            toast(($trad('Error de IA: ')) + msg.slice(0, 80), 'error');
        } finally {
            loading = false;
            if (timerInterval) { clearInterval(timerInterval); timerInterval = null; }
        }
                                    ...
                                    <div class="skill-card" class:skill-pinned={favorites.has(skill.id)}>
                                        <div class="skill-header">
                                            ...
                                            <div class="skill-status">
                                                <button class="btn-fav" class:fav-active={favorites.has(skill.id)}
                                                    ...
                                                    title={favorites.has(skill.id) ? ($trad('Quitar favorito')) : ($trad('Marcar favorito'))}>
                                        ...
                                        <div class="skill-meta">
                                            ...
                                            {#if skill.last_executed}
                                                <span>{lang.lastExecuted}: {new Date(skill.last_executed).toLocaleDateString($trad('es-MX'))}</span>
                                            {/if}
                                        </div>
...
<ConfirmModal
    ...
    variant="danger"
    title={lang.delete}
    message={$trad('¿Eliminar esta skill? Esta acción no se puede deshacer.')}
