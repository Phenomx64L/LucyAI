    ...
    if (lines[i].includes('const results = await Promise.allSettled(readOnlyTasks.map(t2 => t2.fn()));')) {
        if (!lines[i-1].includes('Error Sintaxis')) {
            lines[i] = `                        if (lucyText.match(/<TOOL>.*?<\\/TOOL>/i)) {
                            ...
                            unparsed.forEach(up => {
                                let upN = up[1].trim();
                                readOnlyTasks.push({ label: \`[❌ Error Sintaxis]\`, fn: () => Promise.resolve(\`[SYNTAX ERROR] Se detectó '<TOOL>\${upN}</TOOL>' pero tiene formato inválido o herramienta desconocida.\`) });
...
` + lines[i];
    ...
    if (lines[i].includes("statusLine = `\\n\\n> 🟨 **Se alcanzó el límite de ${MAX_LOOPS} pasos**")) {
...
console.log('Script lines executed safely');
