...
#[tauri::command]
pub async fn call_mcp_tool(server_name: String, query: String) -> Result<String, String> {
    // query = "tool_name|||args_json"
    ...
    let base_cmd = cmd_parts.next().unwrap_or("npx").to_string();
    let mut args_vec: Vec<String> = cmd_parts.map(|s| s.to_string()).collect();

    // If it's pure "npx", auto-inject -y for safety
    if base_cmd == "npx" && !args_vec.contains(&"-y".to_string()) {
        args_vec.insert(0, "-y".to_string());
    }

    let mut child = Command::new(&base_cmd)
        .args(&args_vec)
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .stderr(Stdio::null())
    ...
    // 1. Send Initialize Protocol JSON-RPC
    let init_req = json!({
        "jsonrpc": "2.0",
        "id": 1,
        "method": "initialize",
    ...
    loop {
        buf.clear();
        if let Ok(0) = tokio::time::timeout(std::time::Duration::from_secs(10), reader.read_line(&mut buf)).await.unwrap_or(Ok(0)) {
            let _ = child.kill().await;
            return Err("Timeout o desconexión esperando Initialize de MCP".into());
        }
    ...
    // Initialize notification (required by MCP Spec)
    let init_notif = json!({
        "jsonrpc": "2.0",
    ...
    if result_output.is_empty() {
        Ok("Sin retorno o resultado vacío del conector MCP.".into())
    } else {
        Ok(result_output)
    }
}
